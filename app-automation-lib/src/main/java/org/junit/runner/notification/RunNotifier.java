...
public class RunNotifier {
    ...
    public void fireBeforeGroupStarted(final Description description) {
        new SafeNotifier() {
            @Override
            protected void notifyListener(RunListener each) throws Exception {
                each.beforeGroupStarted(description);
            }
        }.run();
    }

    public void fireAfterGroupStarted(final Description description) {
        new SafeNotifier() {
            @Override
            protected void notifyListener(RunListener each) throws Exception {
                each.afterGroupStarted(description);
            }
        }.run();
    }

    public void fireFixtureFinished() {
        new SafeNotifier() {
            @Override
            protected void notifyListener(RunListener each) throws Exception {
                each.fixtureFinished();
            }
        }.run();
    }

    public void fireFixtureFailed(final Failure failure) {
        new SafeNotifier() {
            @Override
            protected void notifyListener(RunListener each) throws Exception {
                each.fixtureFailed(failure);
            }
        }.run();
    }
